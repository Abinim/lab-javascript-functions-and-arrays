...
function sum(arr) {
  ...
  for (const item of arr) {
    if (typeof item === 'number') {
      ...
      throw new Error('Error: Array contains an invalid data type.');
    }
  }
  return total;
}
...
function averageWordLength(words) {
  ...
  // Calculate the total length of words
...
function avg(arr) {
  ...
  let total = 0;
  // Calculate the total based on element types
  for (const item of arr) {
    if (typeof item === 'number') {
      total += item;
    } else if (typeof item === 'string') {
